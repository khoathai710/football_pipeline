...
class SparkIOManager(IOManager):
    ...
    def _get_path(self, context: Union[InputContext, OutputContext]):
        ...
        if context.has_asset_partitions:
            partition_key = "/".join([key,context.partition_key])
            return f"{partition_key}.parquet", tmp_file_path
        else:
            return f"{key}.parquet", tmp_file_path

    def handle_output(self, context: OutputContext, obj: pd.DataFrame):
        # Convert DataFrame to parquet format
        context._log.info("Upload data frame")
    ...
    def load_input(self, context: InputContext) -> pd.DataFrame:
        context.log.info("Loading data frame")
        key_name, tmp_file_path = self._get_path(context)
        try:
